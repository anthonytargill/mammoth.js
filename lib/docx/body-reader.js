...
function BodyReader(options) {
  ...
  function readRunProperties(element) {
    return readRunStyle(element).map(function (style) {
      ...
      return {
        ...
        // CHANGE HERE - surface the isHidden property
        isHidden: element.firstOrEmpty('w:vanish').name ? true : null,
  ...
  function readColor(element) {
    if (element) {
      ...
      // CHANGE HERE - force auto to be null
      if(value === 'auto'){
        return 'auto'
      }
      return `#${value}`;
      ...
      return readParagraphStyle(element).map(function (style) {
        return {
          ...
          numbering: readNumberingProperties(
            ...
            numbering,
            // CHANGE HERE - passing styles through
            styles
          ),
          indent: readParagraphIndent(element.firstOrEmpty("w:ind")),

          // CHANGE HERE - section details
          sectPr: element.firstOrEmpty("w:sectPr").name ? true : null,

          // CHANGE HERE - pull out the hidden property
          isHidden: element.firstOrEmpty("w:rPr").firstOrEmpty('w:vanish').name ? true : null,
...
// CHANGE HERE - recurively go through styles and pick
// out the numbering
// Issue here is that if a numbering Id is specified in a style, and it's not present in the 
// numbering, then it continues to look through the inheritance chain rather than giving up
// So, we need to ask it - is there a numbering ID? and if so, return that style - or go down the inheritance chain
// Note might need to use the get style from id thing instead as well where the styks
function findNumberingThroughStyles(styleId, stylesPass, numbering, level){
  const style = stylesPass.findParagraphStyleById(styleId)

  // If there's no style, then we can't find it
  if(!style){
    return {
      numIdFromStyle: null,
      numLevelFromStyle: null
    }
  }

  let styleNumberingLevel = level ? level : style.numberingLevel
  let styleNumberingId = style.numberingId

  if(styleNumberingLevel && styleNumberingId){
    return {
        numIdFromStyle: styleNumberingId,
        numLevelFromStyle: styleNumberingLevel
    }
  }
  ...
  return findNumberingThroughStyles(style.basedOnStyleId, stylesPass, numbering, styleNumberingLevel)
}

function readNumberingProperties(styleId, element, numbering, stylesPass) {
  // CHANGE - we need to see if there is a local instance of the numId to use
  // We need to build up the level and numId. First, check to see if we have 
  // both on the element itself?
  var level = element.firstOrEmpty("w:ilvl").attributes["w:val"];
  var numId = element.firstOrEmpty("w:numId").attributes["w:val"];

  if(level && numId){
      const numberingFromElement = numbering.findLevel(numId, level)
      // Note. we don't continue to look in the styles - as sometimes it's 
      // deliberate that a numId doesn't exist, and so we don't have numbering (e.g. set to "0")
      if(!numberingFromElement){
        return null
      }
      return {
        numberingId: numId,
        numberingIsFromStyle: false,
        ...numberingFromElement,
      };
  }
  
  // If we don't have the numId here, but we do have a style to look at
  // then try and find the numId via the style hierarchy. Note - if a 
  // level was specified, then this will override the one found via the style
  if (!numId && styleId) {
    const {
      numIdFromStyle,
      numLevelFromStyle
    } = findNumberingThroughStyles(styleId, stylesPass, numbering, level)

    if (numIdFromStyle && numLevelFromStyle) {
      // We have both a level and numId from the styles, so look up the 
      // style from that
      const numberingFromFoundStyleAttr = numbering.findLevel(numIdFromStyle, numLevelFromStyle)
      if(!numberingFromFoundStyleAttr){
        return null
      }
      return {
        numberingId: numIdFromStyle,
        numberingIsFromStyle: true, // We need this as it affects the order of indents later on.
        // Note. This indent hierarchy is not perfect atm, but will be ok I think for our purposes
        // Ideally, we'd resolve it once we have a full picture of inheritance - but that would
        // require a rewrite of the numbering - where we just dump the numbering in the return along
        // with the styles and resolve it all then. At this point, it would make sense just to 
        // write our own passer for the whole thing
        ...numberingFromFoundStyleAttr,
      };
    }
  }

  // Finally, we can look at the numbering through styles mechanism which is where the numbering specifies the
  // the style directly rather than the other way round. There's a method for this. Note. it doesn't return
  // the numbering Id - but that's fine
  if(styleId){
    const numberingFromNumberingToStyle = numbering.findLevelByParagraphStyleId(styleId)
    if(numberingFromNumberingToStyle){
      return {
        ...numberingFromNumberingToStyle,
        numberingIsFromStyle: true
      }
    }
  }
