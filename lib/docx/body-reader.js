...
function findNumberingThroughStyles(styleId, stylesPass, numbering){
  const style = stylesPass.findParagraphStyleById(styleId)
  if(!style){
    return null
  }

  if(style.numberingId){
    return numbering.findLevelByParagraphStyleId(styleId);
  }
  ...
  return findNumberingThroughStyles(style.basedOnStyleId, stylesPass, numbering)
}
...
function readNumberingProperties(styleId, element, numbering, stylesPass) {
  ...
  if (numId === undefined && styleId != null) {
    var levelByStyleId = findNumberingThroughStyles(styleId, stylesPass, numbering)
    if (levelByStyleId != null) {
      ...
      levelByStyleId.numberingIsFromStyle = true
