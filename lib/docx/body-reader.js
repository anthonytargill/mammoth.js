...
function BodyReader(options) {
    ...
    function readParagraphSpacing(element) {
        return {
            ...
            end: element.attributes["w:after"]
        };
    }
    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function(style) {
            ...
            if(!fontSize){
              ...
              fontSize = /^[0-9]+$/.test(fontSizeString) ? parseInt(fontSizeString, 10) / 2 : null;
            }
            ...
            return {
                ...
                // CHANGE HERE to surface the color property and hasLastRenderedPageBreak
                fontColor: readColor(element.first("w:color"))
               // hasLastRenderedPageBreak: element.first("w:lastRenderedPageBreak") ? true : undefined
            };
        });
    }
    ...
    function readColor(element) {
        if (element) {
            ...
            return value === 'auto' ? '#000000' : `#${value}`
        } else {
            return undefined;
        }
    }
    ...
    function readTable(element) {
        ...
        tableProperties.value.tblPr = element.firstOrEmpty("w:tblPr")
        tableProperties.value.tblGrid = element.firstOrEmpty("w:tblGrid")
...
var ignoreElements = {
    ...
    "w:sectPr": true,
    "w:proofErr": true,
    //"w:lastRenderedPageBreak": true, include this now!?
