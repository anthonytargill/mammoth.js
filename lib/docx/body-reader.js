...
// CHANGE HERE - added a recursive function to find numbering down the tree
function findNumberingDownStyleString(styleId){
  const style = styles.findParagraphStyleById(styleId)
  if(!style){
    return null
  } 
}

function readNumberingProperties(styleId, element, numbering, styles) {
  ...
  if (numId === undefined && styleId != null) {
    ...
    // The numbering might be from a derived style - so let's check 
