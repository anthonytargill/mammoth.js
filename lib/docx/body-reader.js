    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function(style) {
            ...
            return {
                ...
                // CHANGE HERE - read the vanish property http://officeopenxml.com/WPtextFormatting.php
                // TODO - need to know if this needs supporting
                // Which means it should be hidden when rendered
                //isHidden: JSON.stringify(element)
            };
        });
    }
        ...
        "w:pPr": function(element) {
            return readParagraphStyle(element).map(function(style) {
                return {
                    ...
                    // CHANGE HERE - read the vanish property http://officeopenxml.com/WPtextFormatting.php
                    // Which means it should be hidden when rendered
                    isHidden: element.firstOrEmpty("w:rPr").first('w:vanish') ? true : null
                };
            });
        },
...
function readNumberingProperties(styleId, element, numbering) {
    ...
    if (styleId != null) {
        ...
        if (levelByStyleId != null && numId != null) {
            return {
                numberingId: numId,
                ...levelByStyleId
            }
    ...
    if (level === undefined || numId === undefined) {
        ...
        // TODO - need to understand this better - in what circustance does it not exist?
        // example in docx-sample-rfp1.docx search for 'approvals'
        const numberingResult =numbering.findLevel(numId, level)
        if(!numberingResult){
            console.log('numberingResult is null')
            return null
        }
        return {
            // CHANGE HERE - we need to include the 'instance' Id of the numbering
            numberingId: numId,
            ...numberingResult
        }
    }
