...
function findNumberingThroughStyles(styleId, stylesPass, numbering, level){
  ...
  // Maybe we can find the numbering directly from the style the other way round
  const numberingByStyle = numbering.findLevelByParagraphStyleId(styleId);
...
function readNumberingProperties(styleId, element, numbering, stylesPass) {
  ...
  // CHANGE - fishing out the numbering based on the style hierarchy
  if (!numId && styleId) {
    const {
      numIdFromStyle,
      numLevelFromStyle
    ...
    if (numIdFromStyle && numLevelFromStyle) {
      // CHANGE HERE - note. we use this for the inheritance chain for the indents. 
      // Really.. we should know if the style indent overrides the
      if(!numbering.findLevel(numIdFromStyle, numLevelFromStyle)){
        return null
      }
      return {
        numberingId: numIdFromStyle,
        numberingIsFromStyle: true,
        ...numbering.findLevel(numIdFromStyle, numLevelFromStyle),
      };
    }
  }
  ...
  if (level === undefined || numId === undefined) {
    ...
    if(!numbering.findLevel(numId, level)){
      return null
    }
    return {
