...
function readString(xmlString, namespaceMap) {
    ...
    // CHANGE HERE to create an element index map
    var elementIndexMap = {
        absoluteIndex: 0
    };
    ...
    parser.onopentag = function(node) {
        ...
        // CHANGE HERE to fill in the element map and to set the absolute index
        var elementName = mapName(node)
        if(elementIndexMap[elementName] === undefined){
            elementIndexMap[elementName] = 0
        }
        element.elementIndex = elementIndexMap[elementName]++
        element.absoluteIndex = elementIndexMap.absoluteIndex++
